...
run_experiments() {
	...
	local -r con_ng=$((NWORKERS*8)) #180
	# local -r con_g=$NWORKERS #18
	local -r con_g=$((NWORKERS*3)) #18*4 FULL LOAD

	local -r rps_ng=$((1000000*con_ng/DEADLINE_US)) #9000
	local -r rps_g=$((1000000*con_g/DEADLINE_US))   #3600
	...
	for ru in "${MTDBF_RESERVATIONS[@]}"; do
		...
		loadtest -t $DURATION_sec -c $con_g --rps $rps_g -P $ARG "http://${hostname}:${port}" > "$results_directory/$workload_g.dat" 2> /dev/null &
		app_g_PID="$!"
